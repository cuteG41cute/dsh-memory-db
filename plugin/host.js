// ============================================================================
// memory-db 项目记忆库 (Host half) — pkg-26
// 标题语义调整:
// ① 会话标题变化 => 该会话全部条目的 sessionTitle 同步更新为最新标题
// ② 标题变更历史(titleHistory)以「于某时变更为」表达展示
// ③ 模型检索/注入时展示命中会话的标题变更历史
// ============================================================================
...
return {
  ...
  async apply(ctx) {
    ...
    const tableEnabled = (workspaceId) => {
    ...
    const readDefaultEnabled = () => {
      const d = tableEnabled(undefined)
    ...
    async function refreshProject(wpath) {
      ...
      const resolved = (ws ? tableEnabled(ws.id) : undefined) ?? cfg.enabled ?? DEFAULT_ENABLED
    ...
    async function setProjectEnabled(wpath, enabledNow) {
      ...
      const cfg = await readConfig(wpath)
      if (table !== null && ws) {
        await table.put(ws.id, { enabled: enabledNow })
      } else if (fs) {
    ...
    harness.handle('open-admin', async () => {
      try {
    ...
    const memorySearchTool = harness.defineTool({
      name: 'memory_search',
    ...
    harness.registerTool(ctx, memorySearchTool)

    const memoryAdminTool = harness.defineTool({
      name: 'memory_admin',
    ...
    harness.registerTool(ctx, memoryAdminTool)
    ...
    harness.handle('state', async (args) => {
      ...
      const resolved = ws ? (tableEnabled(ws.id) ?? ((configCache.get(wpath) || {}).enabled) ?? DEFAULT_ENABLED) : readDefaultEnabled()
      let title = null
      ...
      return {
        ...
        title: title,
        enabled: resolved,
        defaultEnabled: readDefaultEnabled(),
        storage: table === null ? (openError || 'unavailable') : 'ok',
      }
    })
    harness.handle('set-enabled', async (args) => {
      const wsId = args && typeof args.workspaceId === 'string' && args.workspaceId !== '' ? args.workspaceId : undefined
      const enabledNow = args && typeof args.enabled === 'boolean' ? args.enabled : undefined
    ...
    harness.handle('get-default', async () => {
      return { ok: true, enabled: readDefaultEnabled(), storage: table === null ? (openError || 'unavailable') : 'ok' }
    })
    harness.handle('set-default', async (args) => {
      const enabledNow = args && typeof args.enabled === 'boolean' ? args.enabled : undefined
      if (enabledNow === undefined) return { ok: false, reason: 'bad args' }
      if (table === null) return { ok: false, reason: openError || 'storage unavailable' }
      await table.put(KEY_DEFAULT, { enabled: enabledNow })
      return { ok: true, enabled: enabledNow }
    })
